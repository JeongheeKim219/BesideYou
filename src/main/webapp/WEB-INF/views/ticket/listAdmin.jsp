						...
						<div class="col-md-8 px-md-0 color-white" data-zanim-timeline="{}"
							...
							<div class="overflow-hidden">
								<h1 class="color-white fs-4 fs-md-5 mb-0 zopacity"
									data-zanim='{"delay":0}'>Counseling Tickets</h1>
				...
				<div class="row">
					<c:choose>
						<c:when test="${empty requestScope.tkList.content}">
							<div>
								<p align="center">
									<b><span style="font-size: 15pt;">등록된 상담권이 없습니다.</span></b>
								</p>
							</div>
						</c:when>
						<c:otherwise>
								...
								<c:choose>
									<c:when test="${requestScope.tkList.hasPrevious()==true}">
										<li class="page-item"><a class="page-link"
											href="${pageContext.request.contextPath}/ticket/list?nowPage=${previous}"
								...
								<c:forEach begin="0" end="${tkList.totalPages-1}" var="i">
									...
									<li class="page-item"><c:choose>
											<c:when test="${tkList.number==i}">
												...
												<a class="page-link"
													href="${pageContext.request.contextPath}/ticket/list?nowPage=${i}"
													style="color: red">${i+1}</a>
												<!-- 현재 페이지만 빨간색으로 표시 -->
											</c:when>
											<c:otherwise>
												<a class="page-link"
													href="${pageContext.request.contextPath}/ticket/list?nowPage=${i}">${i+1}</a>
											</c:otherwise>
										</c:choose></li>
								</c:forEach>
								<c:choose>
									<c:when test="${requestScope.tkList.hasNext()==true}">
										<li class="page-item"><a class="page-link"
											href="${pageContext.request.contextPath}/ticket/list?nowPage=${next}"
