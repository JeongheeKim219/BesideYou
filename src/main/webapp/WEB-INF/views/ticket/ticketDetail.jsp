						...
						function() {
							if ($("#remain").text() == 0) {
								...
								if (confirm("상담권을 사용하시겠습니까?")) {
									$("#useticket")
											.attr("action",
													"${pageContext.request.contextPath}/counsel/apply");
									$("#useticket").submit();
								} else {
									return;
								}
					...
					<div
						...
						<div class="d-flex align-items-center h-100">
							<div data-zanim-timeline="{}" data-zanim-trigger="scroll">
								<div class="overflow-hidden" align="right">
									<c:choose>
										<c:when test="${ticket.ticketField==0}">
											<h3 data-zanim='{"delay":0}'>
												<span class="color-1 fs-3 icon-Business-ManWoman"></span>&nbsp;&nbsp;대면
												상담권
											</h3>
										</c:when>
										<c:when test="${ticket.ticketField==1}">
											<h3 data-zanim='{"delay":0}'>
												<span class="color-1 fs-3 icon-Phone-2"></span>&nbsp;&nbsp;전화
												상담권
											</h3>
										</c:when>
										<c:when test="${ticket.ticketField==2}">
											<h3 data-zanim='{"delay":0}'>
												<span class="color-1 fs-3 icon-Phone-SMS"></span>&nbsp;&nbsp;채팅
												상담권
											</h3>
										</c:when>
										<c:when test="${ticket.ticketField==3}">
											<h3 data-zanim='{"delay":0}'>
												<span class="color-1 fs-3 icon-File-TXT"></span>&nbsp;&nbsp;간편
												텍스트 상담권
											</h3>
										</c:when>
									</c:choose>
								...
								<div class="overflow-hidden" align="right">
									<div class="mt-3" data-zanim='{"delay":0.1}'>
										<h5>${counselor.member.name}</h5>
									</div>
									<div class="mt-3" data-zanim='{"delay":0.2}'>
										<fmt:parseDate var="ticketDate" pattern="yyyy-MM-dd'T'HH:mm"
											value="${ticket.ticketDate}" type="both" />
								...
								<br>
								<hr style="height: 1px;">
								<div class="overflow-hidden" align="left"
									style="float: left; margin-right: 150px">
									<div data-zanim='{"delay":0.5}'>
										<c:choose>
											<c:when test="${refundState==0}">
												<span style="font-size: 18px; color: red">환불 처리 진행 중</span>
											</c:when>
											<c:when test="${refundState==1}">
												<span style="font-size: 18px; color: red">환불 불가</span>
											</c:when>
											<c:when test="${refundState==2}">
												<span style="font-size: 18px; color: blue">환불 완료</span>
											</c:when>
										</c:choose>
									</div>
								</div>
								...
								<div class="overflow-hidden" align="right">
									<div data-zanim='{"delay":0.5}'>
										<c:choose>
											<c:when test="${refundState==0}">
												<a href="${pageContext.request.contextPath}/ticket/mylist"><input
													type="button" id="mylist" value="목록으로"
													class="btn btn-outline-primary btn-xs" /></a>
											</c:when>
											<c:when test="${refundState==1}">
												<a href="${pageContext.request.contextPath}/ticket/mylist"><input
													type="button" id="mylist" value="목록으로"
											...
											<c:otherwise>
												<form name="useticket" id="useticket" method="post"
													action="">
													<input type="hidden" name="counselCategory"
														value="${ticket.ticketField}" /> <input type="hidden"
														name="ticketRemain" value="${ticket.ticketRemain}" /> <input
														type="hidden" name="counselorCode"
														value="${counselor.counselorCode}" /> <input type="hidden"
														name="${_csrf.parameterName}" value="${_csrf.token}">
													<input type="button" id="usetk" value="사용하기"
														class="btn btn-outline-primary btn-sm" />&nbsp;&nbsp;&nbsp;

													<!-- <input type="button" id="use" value="사용하기" class="btn btn-outline-primary btn-sm"/>&nbsp;&nbsp;&nbsp; -->
													<input type="button" id="refund" value="환불신청"
														class="btn btn-outline-primary btn-sm" />&nbsp;&nbsp;&nbsp;
													<a href="${pageContext.request.contextPath}/ticket/mylist"><input
														type="button" id="mylist" value="목록으로"
