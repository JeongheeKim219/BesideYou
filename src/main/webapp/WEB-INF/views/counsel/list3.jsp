								...
								<tbody>
									<c:choose>
										<c:when test="${empty pageList.content}">
											<tr>
												<td colspan="5"><h5>아직 예약한 상담이 없습니다.</h5></td>
											</tr>
										</c:when>
										<c:otherwise>
											<c:forEach items="${pageList.content}" var="counsel"
												...
												<tr>
													...
													<td><a class="btn btn-outline-primary btn-xs"
														href="${pageContext.request.contextPath}/requests/read?counselCode=${counsel.counselCode}">상세보기</a></td>
													<c:choose>
															...
															<td>
																<form action="#" method="get">
																	...
																	<input type="hidden" name="${_csrf.parameterName}"
																		...
																		type="hidden" name="counselCode"
																		value="${counsel.counselCode}"> <input
																		type="button" id="reviMo"
																		class="btn btn-outline-primary btn-xs" value="리뷰작성">
																</form>
															</td>
														</c:when>
														<c:otherwise>
