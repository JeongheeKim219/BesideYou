						...
						<tbody>
							<c:choose>
								<c:when test="${empty pageList.content}">
									<tr>
										<td colspan="4">
											<h4 data-zanim='{"delay":0.1}' class="mt-3">자가진단 테스트를
												실행하지 않았습니다.</h4> <a
											href="${pageContext.request.contextPath}/psy/list">자가진단
												하러가기</a>
										</td>
									</tr>
								</c:when>
										...
										<tr>
											...
											<c:choose>
												<c:when test="${list.testName==0}">
													<td>우울증 검사</td>
													<td><a
														href="${pageContext.request.contextPath}/psy/depResult?no=${list.testCode}">${list.total}</a></td>
												</c:when>
												<c:when test="${list.testName==1}">
													<td>스트레스 검사</td>
													<td><a
														href="${pageContext.request.contextPath}/psy/stressResult?no=${list.testCode}">${list.total}</a></td>
												</c:when>
											</c:choose>
											<td><fmt:parseDate var="testDate"
													pattern="yyyy-MM-dd'T'HH:mm" value="${list.testDate}"
													type="both" /> <fmt:formatDate value="${testDate}"
					...
					<ul class="pagination pagination justify-content-center">
						...
						<c:if test="${(startPage-blockCount) >0 }">
							<li class="page-item"><a class="page-link"
								href="${pageContext.request.contextPath}/psy/lo/result?nowPage=${startPage-1}"
						...
						<c:forEach var="i" begin="${startPage}"
							...
							<c:if test="${not doneLoop}">
								<li class="page-item"><a class="page-link" id="page${i}"
									href="${pageContext.request.contextPath}/psy/lo/result?nowPage=${i}">${i}</a>
								</li>
							</c:if>
						...
						<c:if test="${(startPage+blockCount)<=pageList.getTotalPages()}">
							<li class="page-item"><a class="page-link"
								href="${pageContext.request.contextPath}/psy/lo/result?nowPage=${startPage+blockCount}"
