						...
						<tbody>
							...
							<c:choose>
								<c:when test="${empty list}">
									<tr>
										<td colspan="4">
											<h4 data-zanim='{"delay":0.1}' class="mt-3">그림상담을 시작하지
												않았습니다.</h4> <a href="${pageContext.request.contextPath}/psy/art">그림
												상담 시작하기</a>
										</td>
									</tr>
								</c:when>
									...
									<c:forEach items="${list}" var="list" varStatus="state">
										<tr>
											...
											<td><fmt:parseDate var="parseDate"
													...
													type="both" /> <fmt:formatDate value="${parseDate}"
											...
											<c:choose>
												...
												<c:otherwise>
													<td><a href="#" name="replayDone">답변완료</a></td>
										...
										<c:if test="${list.artState!=0}">
											...
											<tr id="${list.artCode}" style="display: none">
												...
												<td><fmt:parseDate var="answerDate"
														pattern="yyyy-MM-dd'T'HH:mm"
														value="${list.artAnswer.artAnsDate}" type="both" /> <fmt:formatDate
														value="${answerDate}" pattern="yyyy년 MM월 dd일" /></td>
											</tr>
